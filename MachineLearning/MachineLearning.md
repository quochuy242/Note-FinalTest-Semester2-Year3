...
$\alpha$ được gọi là *learning rate* và thường giá trị của nó nhỏ. Đây là một trong những *hyperparameter* phổ biến nhất.
...
$$\rm LR = LR * exp(- decayRate \times epochNum)$$2. Step Decay
...
$$x_j ^{(i)} := \frac{x_j ^{(i)} - \micro_j}{s_j}$$
Với $\micro_j = \frac{1}{n} \sum_{i=1}^{n} x_j ^{(i)}$ và $s_j = \sqrt{\frac{\sum_{i=1}^{n} |x_j ^{(i)} - \micro_j|}{n}}$ 
